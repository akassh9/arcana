...
enum CardArt {
  ...
  /// The reverse, pressed into the stock rather than printed on it: a
  /// celestial rose, blind-embossed, that shows only as light and shade.
  /// Mirrored through the centre, so it never tells which way up a card is.
  static func backEmbossMarks() -> [Mark] {
    var p = Pen(seed: 21)
    let cx = 113.0, cy = 200.0
    p.poly([[18, 18], [208, 18], [208, 382], [18, 382]], w: 1.1, amp: 0.7)
    for (rad, w) in [(78.0, 1.0), (62, 1.5), (30, 1.1)] {
      p.ring(cx, cy, rad, w: w, amp: 1.1, segs: 30)
    }
    p.rays(cx, cy, 30, 62, 12, w: 1.0, offset: .pi / 12)
    p.poly([[cx, cy - 94], [cx + 94, cy], [cx, cy + 94], [cx - 94, cy]], w: 0.9, amp: 0.9)
    for sy in [-1.0, 1.0] {
      p.line(58, cy + sy * 130, 168, cy + sy * 130, w: 0.9, amp: 0.6)
      p.diamond(cx, cy + sy * 152, 5, 7, w: 1, c: .ink, amp: 0.4)
    }
    return p.marks
  }

  /// The only gold on the reverse: a hairline border and one sun of leaf.
  static func backGoldMarks() -> [Mark] {
    var p = Pen(seed: 22)
    p.poly([[11, 11], [215, 11], [215, 389], [11, 389]], w: 0.9, c: .gold, amp: 0.5)
    p.dot(113, 200, 11)
    return p.marks
  }

  /// The celestial wheel that turns behind the table. `size` is the
  /// square it is drawn into.
  static func wheelMarks(size n: Double) -> [Mark] {
    var p = Pen(seed: 1_212)
    let c = n / 2
    let u = n / 1500  // everything below is laid out on a 1500 square
    for (rad, w) in [(300.0, 1.4), (318, 0.8), (470, 1.2), (596, 1.6), (610, 0.8), (740, 1.0)] {
      p.ring(c, c, rad * u, w: w * u * 1.4, amp: 2.2 * u, segs: 64)
    }
    // twelve houses
    for i in 0..<12 {
      let a = Double(i) / 12 * .pi * 2
      p.line(
        c + cos(a) * 318 * u, c + sin(a) * 318 * u, c + cos(a) * 596 * u, c + sin(a) * 596 * u,
        w: 1.1 * u * 1.4, amp: 1.6 * u)
      let m = a + .pi / 12
      p.ring(c + cos(m) * 533 * u, c + sin(m) * 533 * u, 7 * u, w: 0.8 * u * 1.4, amp: 0.4 * u, segs: 12)
    }
    p.ticks(c, c, 596 * u, 14 * u, 120, w: 0.8 * u * 1.4)
    p.ticks(c, c, 470 * u, 9 * u, 72, w: 0.8 * u * 1.4)
    // a star of eight in the inner court
    for k in 0..<2 {
      let o = Double(k) * .pi / 4
      let pts = (0..<4).map { i -> [Double] in
        let a = o + Double(i) / 4 * .pi * 2 - .pi / 2
        return [c + cos(a) * 300 * u, c + sin(a) * 300 * u]
      }
      p.poly(pts, w: 1.0 * u * 1.4, amp: 1.8 * u)
    }
    p.ring(c, c, 124 * u, w: 1.0 * u * 1.4, amp: 1.4 * u, segs: 40)
    p.rays(c, c, 124 * u, 300 * u, 16, w: 0.7 * u * 1.4)
    return p.marks
  }

  // --- caches: the pen is deterministic, so draw each set only once ---

  @MainActor private static var artCache: [ArtKind: [Mark]] = [:]
  @MainActor static let frame: [Mark] = frameMarks()

  @MainActor static func art(_ kind: ArtKind) -> [Mark] {
    if let m = artCache[kind] { return m }
    let m = marks(kind)
    artCache[kind] = m
    return m
  }

  // --- a card given back: what the pen leaves in the stock ------------

  /// The marks heavy enough to leave a groove once their ink has sunk:
  /// the strong lines, and the edges of the solid shapes — a filled diamond
  /// is pressed as its outline, as the diamonds on the reverse are.
  /// Hatching, hairlines, ticks and fine rays sink without a trace.
  static func pressed(_ marks: [Mark]) -> [Mark] {
    marks.compactMap { m in
      if m.fill == .ink || m.fill == .accent {
        return Mark(path: m.path, stroke: .ink, width: 1.0, fill: nil)
      }
      guard m.stroke != nil, m.width >= 1.2 else { return nil }
      var out = m
      out.fill = nil
      return out
    }
  ...
  /// The leaf, which does not sink: the gold, and the paper that cuts it
  /// (the Priestess's crescent), in the order they were laid. Gold drawn on
  /// top of gold — the Sun's eye — was painted on the leaf, not laid as
  /// leaf, so it sinks with the ink.
  static func leaf(_ marks: [Mark]) -> [Mark] {
    var laid: [CGRect] = []
    return marks.compactMap { m in
      guard m.fill == .gold || m.fill == .paper else { return nil }
      if m.fill == .gold {
        let r = m.path.boundingRect
        if laid.contains(where: { $0.contains(r) }) { return nil }
        laid.append(r)
      }
      var out = m
      out.stroke = nil
  ...
  @MainActor private static var pressedCache: [ArtKind: [Mark]] = [:]
  @MainActor private static var leafCache: [ArtKind: [Mark]] = [:]
  @MainActor static let pressedFrame: [Mark] = pressed(frame)

  @MainActor static func pressedArt(_ kind: ArtKind) -> [Mark] {
    if let m = pressedCache[kind] { return m }
    let m = pressed(art(kind))
    pressedCache[kind] = m
    return m
  }

  @MainActor static func leafArt(_ kind: ArtKind) -> [Mark] {
    if let m = leafCache[kind] { return m }
    let m = leaf(art(kind))
    leafCache[kind] = m
    return m
  }
